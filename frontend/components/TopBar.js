    ...
    <header className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-8 shrink-0">
      ...
      <div className="flex items-center gap-4">
        <button className="relative w-10 h-10 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200/40 flex items-center justify-center text-lg transition" onClick={onToggleNotifications}>
          🔔
          {unreadCount > 0 && (
            <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white font-extrabold text-[10px] rounded-full flex items-center justify-center border-2 border-white">
              {unreadCount}
            </span>
          )}
        </button>
        <div className="w-9 h-9 rounded-full bg-healthcare-100 text-healthcare-700 text-xs font-bold uppercase flex items-center justify-center border border-healthcare-200">
          {user.firstName?.[0] || "U"}
        </div>
      </div>
    </header>
